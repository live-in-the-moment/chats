...
void CreatTable(sqlite3 *ppdb)
{
    // 创建表
    char sq1[128] = {0};
    sprintf(sq1, "create table if not exists mytable(id char,passwd char);");
...
void MsgSendRecv(thread_node *node)
{
    ...
    {
        // 注册
        if (strcmp(RecvInfo.header.message_type, "REG") == 0)
        {
            Register(node, &RecvInfo);
        }
        // 登录
        else if (strcmp(RecvInfo.header.message_type, "LOGIN") == 0)
        {
            Login(node, &RecvInfo);
        }
        // 查看在线用户
        else if (strcmp(RecvInfo.header.message_type, "LOOKUSERS") == 0)
        {
        ...
        // 群聊
        else if (strcmp(RecvInfo.header.message_type, "GROUP") == 0)
        {
            if (-1 == InspectOwnOnline(node))
            {
        ...
        // 私聊
        else if (strcmp(RecvInfo.header.message_type, "PRIVATE") == 0)
        {
            if (-1 == InspectOwnOnline(node))
            {
        ...
        // 查看聊天记录
        else if (strcmp(RecvInfo.header.message_type, "LOOKCHATRECORD") == 0)
        {
        ...
        // 传输文件
        else if (strcmp(RecvInfo.header.message_type, "FILE") == 0)
        {
